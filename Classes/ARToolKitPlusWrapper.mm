...
- (void)setupWithImageBuffer:(CVImageBufferRef)imageBuffer {
	...
	tracker = new ARToolKitPlus::TrackerSingleMarkerImpl<6, 6, 6, 1, 1>(width,height);
    ...
    tracker->setMarkerMode(self.useBCHMarker ? ARToolKitPlus::MARKER_ID_BCH : 
												ARToolKitPlus::MARKER_ID_SIMPLE);
	
    /*We use automatic thresholding, this ways if lighting conditions are changing the tracking 
	 is still accurate*/
...
- (void)detectMarkerInImageBuffer:(CVImageBufferRef)imageBuffer {
	...
	if (markerId!=-1) {
		...
		for (i=0; i<16; i++) {
			[modelViewMatrix 
			 addObject:[NSNumber numberWithFloat:tracker->getModelViewMatrix()[i]]];
		}
	}
	...
	/*We call our delegate*/
	[self.delegate aRToolKitPlusWrapper:self didDetectMarker:markerId 
						 withConfidence:conf
